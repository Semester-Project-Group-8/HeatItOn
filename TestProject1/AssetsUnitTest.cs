...
public class AssetServiceTests : IDisposable
{
    ...
    private AssetsService _assetService;
    ...
    [Fact]
    public async Task GetExcistingAsset()
    {
        var image = new Image { Id = 1, ImageLink = "test.png" };
        _context.Images.Add(image);
    ...
    [Fact]
    public async Task AddAssets_EmptyList_ThrowsArgumentException()
    {
        await Assert.ThrowsAsync<ArgumentException>(async () => await _assetService.AddAssets(new List<Asset>()));
    }

    [Fact]
    public async Task AddAssets_NullList_ThrowsArgumentException()
    {
