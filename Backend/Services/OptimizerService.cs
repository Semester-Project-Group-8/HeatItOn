...
public class OptimizerService
{
    ...
    public OptimizerService(AssetsService assetService, SourceService sourceService, ResultService resultService,
        ResultByHourService resultListService, OptimizedResultsService optimizedResultsService)
    {
    ...
    {
        var assetProductionCapacity = heatTarget / asset.MaxHeat; //determiunes the capacity the asset is running at
        ...
        var netProductionCost = asset.ProductionCost * assetProductionCapacity * asset.MaxHeat;
    ...
    {
        return assets.OrderBy(asset => asset.ProductionCost - asset.MaxElectricity / asset.MaxHeat * electricityPrice)
            .ToList();
    }
    ...
    public async Task<ActionResult<OptimizedResults>> Optimize(List<Asset> scenarioAssets)
    {
        ...
        var allSources = (await _sourceService.List())
            ...
            .ToList();
        var finalResults = new OptimizedResults
        {
            Name = $"result_{DateTime.Now:yyyy_MM_dd_HH_mm_ss}",
            ResultsForHours = new List<ResultByHour>()
        };
        var scenarioNames = new List<string>
            { "Gas boiler 1", "Gas boiler 2", "Gas boiler 3", "Oil boiler 1" }; //check if this is one of the scenarios
        if (scenarioAssets != null && scenarioAssets.Count == 4 &&
            scenarioNames.All(name => scenarioAssets.Any(a => a.Name == name)))
        ...
        if (scenarioAssets != null && scenarioAssets.Count == 4 &&
            scenarioNames.All(name => scenarioAssets.Any(a => a.Name == name))) //check if this os the other scenario
        ...
        foreach (var source in allSources)
        {
                   ...
                   (heatOfTheHour < source.HeatDemand || allowOverproduction))
            {
                ...
                if (source.HeatDemand - heatOfTheHour > scenarioAssets[usedGenerators].MaxHeat ||
                    heatOfTheHour >= source.HeatDemand) //demand higher than asset can generate
                {
                ...
                else //check if we profit by overproduction
                {
                    ...
                    var partialProductionResult =
                        CalculateAssetResult(scenarioAssets[usedGenerators], source, targetHeatProduction);
                    var fullProductionResult = CalculateAssetResult(scenarioAssets[usedGenerators], source,
                        scenarioAssets[usedGenerators].MaxHeat);
                    if (fullProductionResult.ProductionCost < partialProductionResult.ProductionCost)
                        targetHeatProduction = scenarioAssets[usedGenerators].MaxHeat;
                }

                if (PTES != null && scenarioAssets[usedGenerators] == PTES &&
                    PTES.MaxHeat == 0) //if we dont store any heat skipp
                {
                    usedGenerators++;
                    continue;
                }

                var newResult = CalculateAssetResult(scenarioAssets[usedGenerators], source, targetHeatProduction);
                if (heatOfTheHour >= source.HeatDemand &&
                    newResult.ProductionCost >= 0) //if we met demand or if we dont make money by overproducing stop
                    allowOverproduction = false;
                if (allowOverproduction || (PTES != null && scenarioAssets[usedGenerators] == PTES &&
                                            heatOfTheHour < source.HeatDemand))
                {
            ...
            {
                var overProduction = heatOfTheHour - source.HeatDemand;
                if (overProduction > 30) overProduction = 30; //Max transfer is 30 MWh
