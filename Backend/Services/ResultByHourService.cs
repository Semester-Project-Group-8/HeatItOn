...
public class ResultByHourService : IService<ResultByHour>
{
    ...
    public async Task Delete(int id)
    {
        var resultList = await _dbContext.ResultByHours
            .Include(rl => rl.Results)
            .FirstOrDefaultAsync(rl => rl.Id == id);
        if (resultList == null) throw new KeyNotFoundException($"Result list with id {id} not found.");

        if (resultList.Results.Count > 0) _dbContext.Results.RemoveRange(resultList.Results);
